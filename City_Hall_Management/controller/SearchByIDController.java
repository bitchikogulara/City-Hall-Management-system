...
public class SearchByIDController {
    ...
    private String formatCitizenInfo(Citizen c) {
        ...
        if (c instanceof Male male) {
            if (male.getChildren().isEmpty()) {
                sb.append("  No children recorded.\n");
            } else {
                for (Citizen child : male.getChildren()) {
                    sb.append("  ").append(child.getFullName()).append(" (ID: ").append(child.getIdNumber()).append(")\n");
                }
            }
        } else if (c instanceof Female female) {
            if (female.getChildren().isEmpty()) {
                sb.append("  No children recorded.\n");
            } else {
                for (Citizen child : female.getChildren()) {
                    sb.append("  ").append(child.getFullName()).append(" (ID: ").append(child.getIdNumber()).append(")\n");
                }
            }
        }
